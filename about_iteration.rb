...
class AboutIteration < Neo::Koan
  ...
  def test_break_works_with_each_style_iterations
    ...
    result = 0 + 1 + 2 + 3 # Stops when item  == 4
    assert_equal result, sum
  end
  ...
  def test_collect_transforms_elements_of_an_array
    ...
    result = [11,12,13]
    assert_equal result, new_array

    # NOTE: 'map' is another name for the 'collect' operation
    another_array = array.map { |item| item + 10 }
    assert_equal result, another_array
  end
  ...
  def test_inject_will_blow_your_mind
    ...
    # Inject starts with the provided value, then combines it with each entry in the array using the provided equation
  end
  ...
  #   # code to read 'file'
    ...
    # Similar to using, the file value gets "cleaned up" when it leaves the "do" statement.
  #
